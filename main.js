...
function submitHandler(event) {
  ...
  if ((document.querySelector('.tableThing').style.display === "none")) {
    console.log("gafjeao");
    document.querySelector('.tableThing').style.display = "block";
  }
}
...
function addProductToCart(product, quantity) {
  ...
  const existingLine = cartTableBodyElement.querySelector(
  ...
  if (existingLine) {
    ...
    const totalPriceElement = existingLine.querySelector(".totalPriceOfItem");
    if (quantityElement) {
      quantityElement.textContent = existingLine.dataset.quantity;
      totalPriceElement.textContent = formatPrice(existingLine.dataset.quantity * existingLine.dataset.price);
    }
  } else {
...
export function updateTotalPrice() {
  ...
  for (const line of cartRows) {
    const g = line.dataset.price;
    const h = line.dataset.quantity;

    finalSum += g * h;
  }

  // send the result out
  totalPriceElement.textContent = `${finalSum} kr.-`;
}

// dont know if this is needed but ill put it here for good measures
updateTotalPrice();
